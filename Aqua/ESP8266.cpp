...
uint32_t ESP8266::recvBody(char *buffer, uint32_t buffer_size, uint32_t timeout)
{
	String data;
	char a;
	int32_t len = -1;
	...
	while (millis() - start < timeout) {
		...
		int32_t index_PIPDcomma = data.indexOf("+IPD,");
		if (index_PIPDcomma != -1) {
			int32_t index_colon = data.indexOf(':', index_PIPDcomma + 5);
			if (index_colon != -1) {
				int32_t index_comma = data.indexOf(',', index_PIPDcomma + 5);
				/* +IPD,id,len:data */
				if (index_comma != -1 && index_comma < index_colon) {
					int8_t id = data.substring(index_PIPDcomma + 5, index_comma).toInt();
					if (id < 0 || id > 4) {
						return 0;
					}
	...
	const char* successfulResponse = "HTTP/1.1 200";
	String responseCodeData;
	start = millis();
	while (millis() - start < timeout && responseCodeData.length() <= strlen(successfulResponse))
	{
		if (m_puart->available() > 0)
		{
			a = m_puart->read();
		...
		if (responseCodeData.indexOf(successfulResponse) != -1)
		{
			hasSuccessfulResponse = true;
			len -= strlen(successfulResponse);
			break;
		}
	}
	...
	const uint8_t checkSequenceLength = 4;
	uint8_t *checkBuffer = new uint8_t[checkSequenceLength];
	...
	while (millis() - start < 3000)
	{
		while (m_puart->available() > 0 && i < ret)
		{
			...
			if (!(checkBuffer[0] == 13 && checkBuffer[1] == 10 && checkBuffer[2] == 13 && checkBuffer[3] == 10))
			{
				for (uint8_t z = 0; z < checkSequenceLength; z++)
				{
					checkBuffer[z] = checkBuffer[z + 1];
				}
				checkBuffer[checkSequenceLength - 1] = a;
...
bool ESP8266::sATCIPSTARTSingle(String type, String addr, uint32_t port)
{
	...
	String data = recvString("OK", "ERROR", "ALREADY CONNECT", 5000);
...
bool ESP8266::sATCIPSTARTMultiple(uint8_t mux_id, String type, String addr, uint32_t port)
{
	...
	String data = recvString("OK", "ERROR", "ALREADY CONNECT", 10000);
	if (data.indexOf("OK") != -1 || data.indexOf("ALREADY CONNECT") != -1) {
		return true;
	}
	return false;
...
bool ESP8266::sATCIPCLOSEMulitple(uint8_t mux_id)
{
	...
	String data = recvString("OK", "link is not", 5000);
